    ...
    <div class="home-container">
        ...
        <div class="content clearfix">
            ...
            <Table :loading="loading" border :columns="columns" :data="tableData">
                <template slot-scope="{ row, index }" slot="action">
                    ...
                    <Button type="primary" ghost style="margin-right: 10px" @click="complete(row)"
                            v-if="row.status != 2">{{ row.status == 0 ? '完成' : row.status == 1 ? '待办' : null}}
                    </Button>
                    <Button type="error" ghost @click="remove(row.id)" v-if="row.status != 2">删除</Button>
                </template>
            </Table>
            <router-link :to="{ name: 'Test2'}">前往测试页面</router-link>
        </div>
        ...
        >
                ...
                <Row :gutter="32">
                    <Col span="24">
                        <FormItem label="截止日期" prop="date" label-position="top">
                            <DatePicker :editable="false" v-model="formValidate.date" type="date" placeholder="请选择截止日期"
            ...
            <div class="demo-drawer-footer">
                <Button type="primary" style="margin-right: 15px" @click="handleSubmit('formValidate')">{{ textBtn }}
                </Button>
